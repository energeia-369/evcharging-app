...
export default function FleetDashboard() {
  ...
  const { currentDriver, currentVehicle, dashboardMetrics, resetOperations, nxlTokens } = useFleetOps()
  ...
  useEffect(() => {
    if (user?.email) {
      AsyncStorage.getItem(`@energeia_manager_kyc_${user.email}`).then((val) => {
        setManagerKycVerified(val === 'verified')
      })
    }
  }, [user?.email])
  ...
  const quickActions: QuickAction[] = [
    { label: isCustomer ? 'Book EV Cab' : 'Point-to-Point Fleet Service', icon: 'qrcode-scan', route: '/fleet-management/trip-booking' },
    { label: 'Live Tracking', icon: 'map-marker', route: '/fleet-management/tracking' },
    { label: 'Invoice & History', icon: 'file-document', route: '/fleet-management/invoice' },
  ]

  const managerQuickActions: QuickAction[] = [
  ...
  const actionsToDisplay = isCustomer ? quickActions : managerQuickActions

  function handleActionPress(action: QuickAction) {
    if (action.route === '/fleet-management/trip-booking') {
      if (!isCustomer && !managerKycVerified) {
        Alert.alert(
          'Verification Required',
          'Please upload and verify your manager profile documents first.'
        )
        return
      }
      if (!isCustomer && (summary.totalVehicles === 0 || summary.totalDrivers === 0)) {
        Alert.alert(
          'Setup Required',
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        <FleetCard style={styles.heroCard}>
          <View style={styles.heroRow}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={styles.kicker}>{isCustomer ? 'Energeia EV Cab Booking' : 'Connected Fleet Operations'}</Text>
              <Text style={styles.heroTitle}>Welcome back, {user?.fullName ?? user?.email ?? 'User'}</Text>
              <Text style={styles.heroSubtitle}>
                {isCustomer 
                  ? 'Book instant Pune route rides using electric vehicles and earn NXL cashbacks!'
                  : 'Your EV fleet is live, monitored, and ready for trip dispatch.'
                }
              </Text>
            </View>
            <Pressable style={styles.logoutButton} onPress={handleLogout}>
              <MaterialCommunityIcons name={'logout' as any} size={16} color="#0f5132" />
        ...
        {isCustomer && (
          <FleetCard style={styles.walletDisplayCard}>
            <View style={styles.walletDisplayHeader}>
              <MaterialCommunityIcons name="wallet" size={28} color="#10b981" />
              <View>
                <Text style={styles.walletDisplayTitle}>Your NXL Wallet</Text>
                <Text style={styles.walletDisplayBalance}>{nxlTokens} NXL Tokens</Text>
              </View>
            </View>
            <Text style={styles.walletDisplayDesc}>🎁 Use these tokens on your next ride booking to get instant discounts! (1 NXL = ₹1)</Text>
          </FleetCard>
        )}
          ...
          <FleetCard style={styles.healthCard}>
            <View style={styles.healthHeader}>
              ...
              <View style={styles.scoreBadge}>
                <MaterialCommunityIcons name={'dash' as any} size={14} color="#10b981" />
            ...
            <FleetCard style={styles.kycCard}>
              ...
              <View style={styles.kycItemRow}>
                <MaterialCommunityIcons 
                  name={managerKycVerified ? "check-circle" : "circle-outline"} 
                  size={18} 
                  color={managerKycVerified ? "#10b981" : "#9ca3af"} 
                />
                <Text style={styles.kycItemText}>Manager Profile & Documents Upload</Text>
                <Pressable 
                  style={styles.kycActionBtn} 
                  onPress={() => router.push('/fleet-management/manager-kyc')}
                >
                  <Text style={styles.kycActionBtnText}>
                    {managerKycVerified ? "Edit Docs" : "Upload"}
                  </Text>
                </Pressable>
              </View>
              ...
              {managerKycVerified && summary.totalVehicles > 0 && summary.totalDrivers > 0 ? (
                ...
                <View style={styles.kycWarningBanner}>
                  ...
                  <Text style={styles.kycWarningText}>Complete manager profile + vehicle + driver setup to unlock Fleet Service routing.</Text>
                </View>
              )}
            </FleetCard>
          </>
...
const styles = StyleSheet.create({
  ...
  walletDisplayCard: { marginBottom: 16, backgroundColor: '#ffffff', borderRadius: 18, padding: 16, borderWidth: 1, borderColor: '#dbe7dd' },
  walletDisplayHeader: { flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 8 },
  walletDisplayTitle: { color: '#0f5132', fontSize: 13, fontWeight: '700' },
  walletDisplayBalance: { color: '#10b981', fontSize: 18, fontWeight: '900', marginTop: 2 },
  walletDisplayDesc: { color: '#4f6952', fontSize: 11, fontWeight: '700', lineHeight: 16 },
})
